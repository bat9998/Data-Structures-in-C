...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
  ...
  clearVisited(g);
  cirListDeque vertStack;
  initCirListDeque(&vertStack);
  addFrontCirListDeque(&vertStack, source);

  while(!isEmptyCirListDeque(&vertStack)) {
   
    Vertex *cur = frontCirListDeque(&vertStack);
    removeFrontCirListDeque(&vertStack);
    cur->isVisited = 1;
    ...
    int i;
      ...
      if (isAdjacent(cur->neighbors[i], destination)) {
      
        return 1;
      } 
      if (!cur->neighbors[i]->isVisited) {
  
        addFrontCirListDeque(&vertStack, cur->neighbors[i]);
      }
    }
  }
	return 0;
...
{
  clearVisited(g);
  cirListDeque vertQueue;
  initCirListDeque(&vertQueue);
  addFrontCirListDeque(&vertQueue, source);


  while(!isEmptyCirListDeque(&vertQueue)) {
   
    Vertex *cur = backCirListDeque(&vertQueue);
    removeBackCirListDeque(&vertQueue);
    cur->isVisited = 1;
    ...
    int i;
      ...
      if (!cur->neighbors[i]->isVisited) {
        ...
        addFrontCirListDeque(&vertQueue, cur->neighbors[i]);
      }
    }
  }
